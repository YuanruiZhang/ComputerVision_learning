...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        corect_logprobs = -np.log(softmax_matrix [range(N), y])
        ...
        #calculate gradients for softmax matrix
        dscores = softmax_matrix 
        dscores[range(N),y] -= 1
        dscores /= N
        
        # W2 and b2
        grads['W2'] = a1.T @ dscores # [HxN] * [NxC] = [HxC]
        grads['b2'] = np.sum(dscores, axis=0)

        # Propagate to Relu layer
        dhidden = dscores @ W2.T # [NxC] * [CxH] = [NxH]
        # Backprop the ReLU non-linearity
        dhidden[a1 <= 0] = 0 #if > 0 pass on gradient, if <= 0 set gradient 0.
